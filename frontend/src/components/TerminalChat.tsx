...
const HELP = `chef chat — powered by conduit
  /help    show this message
  /clear   clear chat history

example questions:
  what should i cook tonight?
  should i cook or order today?
  what have i been eating this week?
  log that i ordered sushi for dinner`;
...
export function TerminalChat() {
  const [msgs, setMsgs] = useState<Msg[]>([
    { id: uid(), role: "system", content: "chef chat ready. type a question or /help for commands." },
  ]);
    ...
    <>
      ...
      <div className="tc-shell">
        <div className="tc-topbar">
          <span className="tc-brand">chef</span>
        ...
        <div className="tc-input-wrap">
          ...
          <textarea
            ...
            onKeyDown={handleKey}
            placeholder={streaming ? "streaming…" : "what should i cook tonight?"}
            rows={1}
            spellCheck={false}
            autoComplete="off"
          />
        ...
        <div className="tc-statusbar">
          chef · kitchen · {streaming ? "streaming…" : "ready"}
        </div>
      </div>
    </>
  );
