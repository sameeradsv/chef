...
def generate_restaurant_suggestions(
    ...
    cache_key = f"{city.lower()}|{','.join(sorted(cuisines))}|{vegetarian}|{craving.lower()[:30]}"
    now = time.time()
...
- "restaurant_name": string (realistic local name)
- "cuisine": string (e.g. "South Indian", "North Indian", "Chinese", "Italian", "Mughlai")
...
def best_ai_restaurant(
    ...
    """Pick the best restaurant from AI suggestions based on craving and budget match."""
    if not suggestions:
        return None
    best = suggestions[0]
    best_score = -1
    for r in suggestions:
        score = 0.0
        if craving and craving.lower() in r.get("cuisine", "").lower():
            score += 10
