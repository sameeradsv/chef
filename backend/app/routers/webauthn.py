...
from app.models import UserAccountModel, WebAuthnChallenge, WebAuthnCredential
...
def _store(db: Session, challenge: bytes, user_id: Optional[str] = None) -> str:
    now = datetime.utcnow()
...
def login_complete(body: _AuthComplete, db: Session = Depends(get_db)):
    ...
    db.commit()
    # chef user_id is a UUID string — look up directly without int() cast
    user = db.get(UserAccountModel, cred.user_id)
    if not user:
