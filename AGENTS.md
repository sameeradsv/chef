...
- `mealdb.py` — Groq recipe generation (70b for full, 8b for fast) with 30-min in-process cache keyed on pantry contents + meal type + preferences; falls back to TheMealDB live search then seed data. Normal cache pool is ≥10 results; fast mode uses a smaller pool/prompt and dynamic token cap to stay below Groq on-demand 8B TPM limits. Do not use Groq `response_format={"type":"json_object"}` here: recipe prompts can produce provider-side 400s when the model emits fractions/truncated JSON, so Chef asks for JSON in-text, locally repairs common issues like `1/2`, and then parses/falls back.
...
| `app/decision/page.tsx` | Cook vs order vs eat out — Chef/Cortex energy preset, score waterfall; reasoning/narrative/predict/cost-insights collapsed under a RATIONALE toggle below the option cards |
...
- **Vision / screenshot parsing** — `routers/vision.py` calls Groq vision with `CHEF_VISION_MODEL` (default `qwen/qwen3.6-27b`); requires same `GROQ_API_KEY`. Used by pantry photo entry, product-label parsing, and history page screenshot-to-log. The old Llama 4 Scout model was shut down by Groq in July 2026, so do not hard-code it again.
...
The design handoff (`Codex Design/chef-designs/design_handoff_kitchen_intelligence/`) specified certain features that were deliberately **not implemented**. Do not re-add these without explicit approval:
...
- `routers/energy.py` + `routers/sync.py` — cumulative meal-energy timeline (signed deltas, running balance, restorative good meals) and today's drain; `/energy/timeline` feeds Canopy/Circuit cross-app charts, and `/sync/energy` feeds Chef local-account Decide presets
- `frontend/src/lib/cross-app-energy.ts` — presets Decide energy; local Chef accounts use `/sync/energy`, Cortex accounts use client-side combined energy (Circuit start + merged deltas from all three apps)
...
| Laptop / desktop | ≥ 1025 px | Full layout; sidebar nav preferred |
