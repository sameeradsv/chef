...
- `mealdb.py` — Groq recipe generation (70b for full, 8b for fast) with 30-min in-process cache keyed on pantry contents + meal type + preferences; falls back to TheMealDB live search then seed data. Cache pool is always ≥10 results; callers slice to their requested count. Do not use Groq `response_format={"type":"json_object"}` here: recipe prompts can produce provider-side 400s when the model emits fractions/truncated JSON, so Chef asks for JSON in-text, locally repairs common issues like `1/2`, and then parses/falls back.
